...
class EditHouse extends StatefulWidget {
  final GetHouse oldData;
  final List houseMedia;
  const EditHouse({super.key, required this.oldData, required this.houseMedia});
...
class _EditHouseState extends State<EditHouse> {
  ...
  @override
  void initState() {
    price = TextEditingController(text: '${widget.oldData.houseDetails.price}');

    details = TextEditingController(text: widget.oldData.houseDetails.detials);

    phoneNumber = TextEditingController(
        text: '${widget.oldData.houseDetails.phoneNumber}');

    houseName =
        TextEditingController(text: widget.oldData.houseDetails.housesName);

    rent = TextEditingController(text: '${widget.oldData.houseDetails.rent}');

    rooms = TextEditingController(text: '${widget.oldData.houseDetails.rooms}');

    lavatory =
        TextEditingController(text: '${widget.oldData.houseDetails.lavatory}');

    area = TextEditingController(text: '${widget.oldData.houseDetails.area}');

    diningRooms = TextEditingController(
        text: '${widget.oldData.houseDetails.diningRooms}');

    sleepingRooms = TextEditingController(
        text: '${widget.oldData.houseDetails.sleepingRooms}');

    isForSale = widget.oldData.houseDetails.isForSale;

    isForRent = widget.oldData.houseDetails.isForRent;

    houseMediaFiles = [];

    category = widget.oldData.houseDetails.category;

    city = widget.oldData.houseDetails.category;

    houseMedia = widget.houseMedia.cast<String>();
  ...
  void setHouseMedia(List data) {
    for (var e in data) {
      houseMedia.add(e.toString());
    }
  }
                ...
                onTap: () async {
                  ...
                  if (email != null &&
                    ...
                    final house = AddNewHouseModel(
                      ...
                      lavatory: int.parse(lavatory.text),
                      area: int.parse(area.text),
                      diningRooms: int.parse(diningRooms.text),
                      sleepingRooms: int.parse(sleepingRooms.text),
                      rent: double.parse(rent.text),
                      media: houseMedia,
                    );
                    services
                        .updateHouse(
                            widget.oldData.houseDetails.housesId, house)
                        .then((isUploaded) {
